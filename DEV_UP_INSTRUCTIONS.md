...
**Company lens (independent):** Lambda (`lambda`)  
**Innovation:** Useful-GPU-Minute Contract  
**Scaffold batch:** 2026-08-10T0924Z

## Mission

Implement a **real, testable** central mechanism that addresses the bottleneck below. Do **not** claim Lambda affiliation, proprietary access, or production deployment.
...
Meter and optimize time from allocation to productive model work, including provisioning, failures, network stalls, storage load, and job retries—not just rented accelerator time.

## Hard rules (fail closed)

1. **No affiliation theater** — never state or imply Lambda employment, endorsement, or proprietary systems access.
...
File: `src/useful_gpu_minute_contract.py`

- Expand `UsefulGpuMinuteContract` into a complete, self-contained implementation.
- Public API must stay stable enough that tests in `tests/test_useful_gpu_minute_contract.py` can be upgraded (not gutted).
- Include at least:
...
### 3. Strengthen tests
Files: `tests/test_useful_gpu_minute_contract.py`, `tests/test_adversarial.py`
...
- Cloud deploy, customer pilots, proprietary Lambda APIs
